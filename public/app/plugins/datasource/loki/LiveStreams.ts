...
const logger = createMonitoringLogger('datasource.loki.livestreams');
          ...
          mergeMap((error, i) => {
            ...
            if (error.code === 1006 && retryAttempt < 30) {
              if (retryAttempt > 10) {
                ...
                logger.logWarning('Websocket connection is being disrupted', {
                  retryAttempt,
                  errorReason: error.reason,
                  suggestion: 'Consider starting new live tailing again',
                });
              }
              // Retry every 5s
