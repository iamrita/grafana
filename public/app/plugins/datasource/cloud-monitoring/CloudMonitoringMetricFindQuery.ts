...
const logger = createMonitoringLogger('datasource.cloudMonitoring.metricFindQuery');
...
export default class CloudMonitoringMetricFindQuery {
  ...
  async execute(query: CloudMonitoringVariableQuery) {
    try {
      ...
      logger.logError(error instanceof Error ? error : new Error('Could not run CloudMonitoringMetricFindQuery'), {
        queryType: query.selectedQueryType,
      });
      return [];
    }
  }
