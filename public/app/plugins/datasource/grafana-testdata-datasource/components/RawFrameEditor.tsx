...
const logger = createMonitoringLogger('datasource.testdata.rawframe');
...
export const RawFrameEditor = ({ onChange, query }: EditorProps) => {
  ...
  const onSaveFrames = (rawFrameContent: string) => {
    try {
      ...
      if (data) {
        logger.logInfo('Frame data conversion', { original: json, saved: data });
        setError(undefined);
      ...
      logger.logWarning('Error parsing json', { error: String(e) });
