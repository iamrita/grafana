...
const logger = createMonitoringLogger('datasource.cloudwatch.variables');
...
export class CloudWatchVariableSupport extends CustomVariableSupport<CloudWatchDatasource, VariableQuery> {
  ...
  async execute(query: VariableQuery) {
    try {
      ...
      logger.logError(error instanceof Error ? error : new Error('Could not run CloudWatchMetricFindQuery'), {
