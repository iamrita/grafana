...
const logger = createMonitoringLogger('features.dimensions.fileUploader');
...
export const FileUploader = ({ mediaType, setFormData, setUpload, error }: Props) => {
  ...
  const onFileRemove = (file: DropzoneFile) => {
    fetch(`/api/storage/delete/upload/${file.file.name}`, {
      method: 'DELETE',
    }).catch((error) => logger.logError(error instanceof Error ? error : new Error('Cannot delete file'), { fileName: file.file.name }));
  };
