...
const logger = createMonitoringLogger('explore.state.query');
      ...
      newQuerySubscription = newQuerySource.subscribe({
        next(data) {
          ...
          if (exploreState!.scanning) {
            logger.logDebug('Scanning results', { seriesCount: data.series.length });
        ...
        error(error) {
          ...
          logger.logError(error instanceof Error ? error : new Error(String(error)), {
            context: 'runQueries.subscription',
          });
        },
        complete() {
  ...
  async ({ exploreId, absoluteRange }, { dispatch, getState }) => {
    ...
    newQuerySource.subscribe({
      ...
      error(error) {
        ...
        dispatch(changeLoadingStateAction({ exploreId, loadingState: LoadingState.Error }));
        logger.logError(error instanceof Error ? error : new Error(String(error)), {
          context: 'runLoadMoreLogsQueries.subscription',
        });
      },
      complete() {
        dispatch(changeLoadingStateAction({ exploreId, loadingState: LoadingState.Done }));
