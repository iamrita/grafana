...
import { getDragStyles, InlineField, Select, useStyles2, createLogger } from '@grafana/ui';
import {
...
const logsTableWrapLogger = createLogger('LogsTableWrap');
...
export function LogsTableWrap(props: Props) {
  ...
  const toggleColumn = (columnName: FieldName) => {
    if (!columnsWithMeta || !(columnName in columnsWithMeta)) {
      logsTableWrapLogger.logger('failed to get column', { columnsWithMeta });
      return;
    }
