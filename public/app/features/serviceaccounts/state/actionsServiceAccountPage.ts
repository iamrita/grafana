...
export function loadServiceAccount(saUid: string): ThunkResult<void> {
  return async (dispatch) => {
    ...
    try {
      ...
      logger.logError(error instanceof Error ? error : new Error('Failed to load service account'), { saUid });
    } finally {
...
export function loadServiceAccountTokens(saUid: string): ThunkResult<void> {
  return async (dispatch) => {
    try {
      ...
      logger.logError(error instanceof Error ? error : new Error('Failed to load service account tokens'), { saUid });
    }
  };
}
