...
import { Box, createLogger } from '@grafana/ui';
...
const logger = createLogger('DashboardScenePage');
...
export function DashboardScenePage({ route, queryParams, location }: Props) {
  ...
  if (type !== 'snapshot' && (!prevMatch || uid !== prevMatch?.params.uid)) {
    logger.logger('skipRender', false, 'Skipping render during dashboard transition');
    return null;
  }
