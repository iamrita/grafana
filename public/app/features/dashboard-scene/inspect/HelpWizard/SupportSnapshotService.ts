...
const logger = createMonitoringLogger('dashboard-scene.support-snapshot-service');
  ...
  async buildDebugDashboard() {
    ...
    if (snapshot) {
      try {
        ...
        logger.logError(ex instanceof Error ? ex : new Error(String(ex)), { action: 'buildDebugDashboard' });
      }
    }
