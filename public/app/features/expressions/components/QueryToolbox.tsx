...
const logger = createMonitoringLogger('expressions.queryToolbox');
...
export const QueryToolbox = ({ onFormatCode, onExpand, isExpanded, query }: QueryToolboxProps): JSX.Element => {
  ...
  const copyTextCallback = useCallback(async () => {
    try {
      ...
      logger.logError(e instanceof Error ? e : new Error(String(e)), { context: 'copying query to clipboard' });
    }
  }, [query.expression]);
