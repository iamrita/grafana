...
interface PolicyComponentProps {
  ...
  /** Earlier siblings in tree order, used to detect unreachable (dead) branches. */
...
const Policy = (props: PolicyComponentProps) => {
  ...
  const policyIssues = detectPolicyIssues(currentRoute, {
    isDefaultPolicy,
    inheritedReceiver: inheritedProperties?.receiver,
    previousSiblings,
  });
  const configWarnings = policyIssues.map(getPolicyIssueMessage);
    ...
    <Stack direction="column" gap={1.5}>
      ...
      >
        ...
        <div className={styles.gutterIconStack}>
          ...
          {configWarnings.length > 0 && <WarningsGutterIndicator warnings={configWarnings} />}
            ...
            {pageOfChildren.map((child) => {
              ...
              return (
                <Policy
                  ...
                  previousSiblings={allChildPolicies.slice(
                    0,
                    allChildPolicies.findIndex((policy) => policy.id === child.id)
                  )}
                />
              );
            })}
...
const WarningsGutterIndicator: FC<{ warnings: string[] }> = ({ warnings }) => {
  const styles = useStyles2(getStyles);
  ...
  return (
    ...
    >
      <div className={styles.gutterIcon('warning')} data-testid="policy-warnings">
        <Icon name="exclamation-triangle" />
      </div>
    </PopupCard>
  );
...
function getPolicyIssueMessage(issue: PolicyIssue): string {
  switch (issue.kind) {
    case 'contradictoryMatchers':
      return t(
        'alerting.policies.warnings.contradictory-matchers',
        'This policy has contradictory matchers and will never match any alerts.'
      );
    case 'unreachable':
      return t(
        'alerting.policies.warnings.unreachable',
        'This policy is unreachable because an earlier sibling matches first and does not continue.'
      );
    case 'noContactPoint':
      return t(
        'alerting.policies.warnings.no-contact-point',
        'This policy has no contact point and no child policies, so matching alerts will not be notified.'
      );
  }
}
...
const getStyles = (theme: GrafanaTheme2) => ({
  ...
  gutterIcon: (severity: boolean | 'error' | 'warning' = false) => {
